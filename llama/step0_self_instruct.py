...
def self_instruct(count=8):
    ...
    def self_instruct_task(retry_times=10):
        ...
        with lock:
            examples = random.sample(instructions[:length], 2)
        ...
        # print(f'\n\n\n\n{output}\n\n\n{is_suitable}\n\n\n')
        if is_suitable.lower() == 'yes' and 3 < count_words(output) < 500:
            with lock:
...
if __name__ == '__main__':
    ...
    # for i in range(args.count // 10000):
    #     instructions = self_instruct(count=10000)
    #     instructions_list.append(instructions)
    # instructions = self_instruct(count=args.count % 10000)
    instructions = self_instruct(count=args.count)
    instructions_list.append(instructions)
